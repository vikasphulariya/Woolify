...
const EditPost = ({route}) => {
  ...
  const [Type, setType] = useState(route.params.Detailed.data.WoolType);
  const [Number, setNumber] = useState(route.params.Detailed.data.Phone);
  const [Quntity, setQuntity] = useState(route.params.Detailed.data.Quantity);
  const [Total, setTotal] = useState(route.params.Detailed.data.Amount);
  const ValidateInfo = () => {
    let c = true;
    if (Number.length < 10) {
      Snackbar.show({
      ...
      {focused ? (
        <View>
          <Animated.Text
            ...
            className="text-2xl text-black text-center my-5 font-bold">
            Update Sell Post
          </Animated.Text>
      ...
      {focused ? (
        <Animated.View
          ...
          <TouchableOpacity
            ...
            <Text className="text-black text-2xl font-extrabold ">Update</Text>
          </TouchableOpacity>
        </Animated.View>
      ) : null}
    </View>
